...
void Screen_InitializeUI(void)
{
	...
	uibutton[BUTTON_ID_QUIT].y1_ = UI_MIDDLE_AREA_APP_CMD_Y + 2;
...
void Screen_ShowLogo(void)
{
	...
	// work back from lightest/most core-to-phoenix color, to edge.
	...
	while (color_loops < 9 && keep_going == true)
	{
		...
		for (j = 0; j < 240; j++)
		{
			...
			// color some "pixels" randomly, but only after having jumped through colorband looping once
			if (color_loops > 1)
			{
				random_x = R16(RANDOM_NUM_GEN_LOW) % 36;
				random_y = R16(RANDOM_NUM_GEN_LOW) % 45;
				random_color = R16(RANDOM_NUM_GEN_LOW) % 14 + 1; // we don't want to use black or highest color (used for text)
			}
			...
			if (color_loops > 1)
			{
				Text_SetAttrAtXY(22+random_x, 7+random_y, random_color << 4);
			}
			
			//General_DelayTicks(120-j/2); // accelerate speed
			General_DelayTicks(8);
